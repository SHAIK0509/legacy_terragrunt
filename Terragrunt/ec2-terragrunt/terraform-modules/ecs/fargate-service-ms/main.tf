...
module "target_group_listener_rules" {
  source = "../alb/target-group-listener-rules-ms"
...
resource "aws_ecs_service" "this" {
  ...
  desired_count                      = lookup(var.desired_count_map, var.name, var.desired_count_map.default)
...
resource "aws_iam_role" "autoscaling" {
  count              = var.do_autoscaling > 0 ? 1 : 0
  name               = "${var.cluster}-${aws_ecs_service.this.name}-appautoscaling-role"
