...
locals {
  service_name           = "${var.environment}-${var.ecs_cluster_name}"
  ...
  network_lb             = "" // we don't need network_lb unless we have gateway,  this module does not do gatway
...
module "service" {
  for_each = toset(var.services)
  source   = "/Users/vivektrivedi/Desktop/Legacy/Test_Code_TF_TG/legacy_terragrunt/terragrunt/ec2-terragrunt/terraform-modules/ecs/fargate-service-ms"

  aws_region     = var.aws_region
  aws_account_id = var.aws_account_id
  network_lb     = local.network_lb
  command        = lookup(var.commands, each.value)

  do_autoscaling = var.do_autoscaling
  max_capacity   = lookup(var.max_capacity, "${var.environment}-${each.value}")
  min_capacity   = lookup(var.min_capacity, "${var.environment}-${each.value}")
  scale_target   = var.scale_target
  cooldown_secs  = var.cooldown_secs
  external_lb    = module.external_lb[each.value].aws_lb_target_group_arn

  external_domain = "${each.value}.${var.external_domain}"
  ...
  task_cpu    = lookup(var.task_cpu, "${var.environment}-${each.value}")
  task_memory = lookup(var.task_memory, "${var.environment}-${each.value}")
  ...
  load_balancer_associations = jsonencode({
    app = {
      port              = 80
      host_headers      = "${each.value}.${var.environment}.legint.net"
      health_check_path = "/version"
    }
  })
...
module "external_lb" {
  ...
  environment    = var.environment
  cluster_name   = "${var.environment}-${each.value}"
  vpc_id         = module.remote.states.vpc.vpc_id
  aws_account_id = var.aws_account_id
